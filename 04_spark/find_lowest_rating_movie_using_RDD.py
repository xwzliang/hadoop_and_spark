...
# This way we can then add up the ratings for each movie, and the
# total number of ratings for each movie (which lets us compute the average)
def parseInput(line):
    fields = line.split()
...
if __name__ == "__main__":
    ...
    lines = sc.textFile("hdfs:///user/maria_dev/MovieLens_data_100k/u.data")
    ...
    ratingTotalsAndCount = movieRatings.reduceByKey(
        lambda movie1, movie2: (movie1[0] + movie2[0], movie1[1] + movie1[1])
    )
